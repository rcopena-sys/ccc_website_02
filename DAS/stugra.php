...
if (!empty($_GET['search_student']) || !empty($_GET['search_course'])) {
    ...
    if (!empty($search_student)) {
        // Filter by student ID on grades table (aliased as g)
    ...
    if (!empty($search_course)) {
        // Filter by course code on grades table (aliased as g)
    ...
    if (!empty($where_conditions)) {
        // Only show grades for students registered as Psychology in students_db
        $sql = "SELECT g.* FROM grades_db g " .
               "INNER JOIN students_db s ON s.student_id = g.student_id " .
               "WHERE s.programs LIKE '%Psychology%' AND " . implode(' AND ', $where_conditions) .
...
// Get course codes for dropdown (Psychology students only)
$course_codes = [];
$course_sql = "SELECT DISTINCT g.course_code, g.course_title " .
              "FROM grades_db g " .
              "INNER JOIN students_db s ON s.student_id = g.student_id " .
              "WHERE s.programs LIKE '%Psychology%' " .
...
// Get student IDs for dropdown (Psychology students only)
$student_ids = [];
$student_sql = "SELECT DISTINCT g.student_id " .
               "FROM grades_db g " .
               "INNER JOIN students_db s ON s.student_id = g.student_id " .
               "WHERE s.programs LIKE '%Psychology%' " .
...
<head>
    ...
    <style>
        ...
        /* Blue gradient for stats cards to match admin dashboard */
        .stats-card { background: linear-gradient(135deg, #1d4ed8 0%, #2563eb 50%, #3b82f6 100%) !important; color: white; }
    ...
    <div class="container py-5">
        ...
        <div class="mt-5">
            <h4 class="mb-3">
                ...
                <?= $search_performed ? 'Search Results' : 'Psychology Students Grades' ?>
            ...
            <div class="table-responsive">
                <table class="table table-sm table-bordered table-hover">
                    ...
                    if ($search_performed) {
                        ...
                        $all_sql = "SELECT g.* FROM grades_db g " .
                                   "INNER JOIN students_db s ON s.student_id = g.student_id " .
                                   "WHERE s.programs LIKE '%Psychology%' " .
                                   "ORDER BY g.student_id, g.course_code";
                        $all_result = $conn->query($all_sql);
                        $display_data = $all_result ? $all_result->fetch_all(MYSQLI_ASSOC) : [];
                    }
