...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 2) If still not found and no error yet, try to derive minimal info from grades_db
    if (!$student_info && $error_message === '') {
        ...
        if ($checkStmt) {
            ...
            if ($checkResult && $checkResult->num_rows > 0) {
                ...
                if (columnExists($conn, 'signin_db', 'course')) {
                    ...
                    if ($signinStmt) {
                        ...
                        if ($signinResult && ($signinRow = $signinResult->fetch_assoc())) {
            ...
            $error_message = 'Failed to prepare grades lookup query: ' . $conn->error;
        }
    }
...
<head>
    ...
    <style>
        .grade-report { background: #fff; padding: 30px; border-radius: 8px; box-shadow: 0 0 10px rgba(37,99,235,0.25); border: 1px solid #bfdbfe; }
        .table th, .table td { vertical-align: middle !important; }
        .semester-title { background: #eff6ff; padding: 10px; margin-top: 30px; border-radius: 5px; font-weight: bold; color: #1d4ed8; border-left: 4px solid #2563eb; }
