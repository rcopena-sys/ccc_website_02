...
// For DAS, always work with Psychology students only
$base_where = $where_clause
  ? $where_clause . " AND programs LIKE '%Psychology%'"
  : " WHERE programs LIKE '%Psychology%'";
...
try {
    ...
    // 2. Get program-wise counts (Psychology only for DAS)
    $bscs_count = 0;
    $bsit_count = 0;
    
    // Count Psychology students from students_db
    $program_result = $conn->query("SELECT COUNT(*) as total FROM students_db" . $base_where);
    if ($program_result && $row = $program_result->fetch_assoc()) {
      // Reuse $bsit_count variable name to store Psychology count
      $bsit_count = (int)$row['total'];
    }

    $total_students = $bsit_count;

    // 3. Get curriculum count for Psychology by fiscal year
    $curriculum_stats = [];
    $curriculum_result = $conn->query("SELECT fiscal_year, COUNT(DISTINCT program) as count 
      FROM curriculum 
      WHERE program LIKE '%Psychology%' 
    ...
    if ($curriculum_result && $curriculum_result->num_rows > 0) {
        ...
        // Fallback to total distinct Psychology programs
    ...
    // 4. Get enrollment by year and semester (Psychology only)
    $enrollment_result = $conn->query("SELECT academic_year, semester, COUNT(*) as total FROM students_db" . $base_where . " GROUP BY academic_year, semester ORDER BY academic_year, semester");
    ...
    // Get total population from students_db (Psychology only)
    $total_result = $conn->query("SELECT COUNT(*) as total FROM students_db" . $base_where);
    if ($total_result) {
    ...
    // 6. Get gender breakdown (Psychology only)
    $gender_result = $conn->query("SELECT LOWER(gender) as gender, COUNT(*) as total FROM students_db" . $base_where . " GROUP BY gender");
    if ($gender_result) {
    ...
    // 7. Get academic year breakdown (Psychology only)
    $year_result = $conn->query("SELECT academic_year, COUNT(*) as total FROM students_db" . $base_where . " GROUP BY academic_year ORDER BY academic_year");
    if ($year_result) {
    ...
    // 8. Get total population (count distinct Psychology student IDs to avoid duplicates)
    $total_result = $conn->query("SELECT COUNT(DISTINCT student_id) as total FROM students_db" . $base_where);
    if ($total_result) {
        $total_population = (int)$total_result->fetch_assoc()['total'];
    }
    ...
    // 10. Get program enrollment for chart (Psychology only)
    $program_enrollment_result = $conn->query("SELECT programs, COUNT(*) as count FROM students_db" . $base_where . " GROUP BY programs");
...
// By curriculum (Psychology only)
$curriculum = [];
if ($res = $conn->query("SELECT curriculum, COUNT(*) as total FROM students_db" . $base_where . " GROUP BY curriculum")) {
  while ($row = $res->fetch_assoc()) {
    $curriculum[$row['curriculum']] = $row['total'];
  }
}

// By classification (Psychology only)
$classifications = [];
if ($res = $conn->query("SELECT classification, COUNT(*) as total FROM students_db" . $base_where . " GROUP BY classification")) {
  while ($row = $res->fetch_assoc()) {
...
// By programs (Psychology only)
$programs = [];
if ($res = $conn->query("SELECT programs, COUNT(*) as total FROM students_db" . $base_where . " GROUP BY programs")) {
  while ($row = $res->fetch_assoc()) {
...
foreach ($year_levels as $level) {
    ...
    $res = $conn->query("SELECT classification, COUNT(*) as total FROM students_db " . ($where_clause ? $where_clause . " AND programs LIKE '%Psychology%'" : " WHERE programs LIKE '%Psychology%'") . " GROUP BY classification");
    if ($res) {
        while ($row = $res->fetch_assoc()) {
...
<body class="flex bg-blue-100 min-h-screen">
 ...
 <aside id="sidebar" class="w-64 bg-blue-600 text-white p-6 flex flex-col transform transition-transform duration-300 ease-in-out">
    <div class="flex flex-col items-center">
      <img src="das.png" class="rounded-full border-4 border-white" alt="DCI Logo" />
      <h2 class="mt-4 font-semibold text-lg">DAS</h2>
    ...
    <nav class="space-y-4 mt-4">
      ...
      <!-- <a href="feedback.php" class="block py-2 px-4 rounded hover:bg-[#17a5b9] transition-colors duration-200">Feedback</a> -->
      ...
      <div class="relative">
        ...
        <div id="studentDropdownMenu" class="hidden absolute left-0 w-full z-10 flex flex-col bg-white border border-blue-200 rounded shadow-lg mt-1">
          <a href="list.php" class="py-2 px-6 text-blue-700 hover:bg-blue-50 border-b border-blue-200 first:rounded-t last:rounded-b">Student List</a>
          <a href="stugra.php" class="py-2 px-6 text-blue-700 hover:bg-blue-50 border-b border-blue-200">Student Grade</a>
          <a href="stucuri.php" class="py-2 px-6 text-blue-700 hover:bg-blue-50 last:rounded-b">Student Curriculum</a>
        </div>
      </div>
      ...
      <div class="relative">
        ...
        <div id="evaluationDropdownMenu" class="hidden absolute left-0 w-full z-10 flex flex-col bg-white border border-blue-200 rounded shadow-lg mt-1">
          <a href="stueval.php" class="py-2 px-6 text-blue-700 hover:bg-blue-50 border-b border-blue-200 first:rounded-t">Evaluate Student</a>
          <a href="semeva.php" class="py-2 px-6 text-blue-700 hover:bg-blue-50 last:rounded-b">Semestral Evaluation</a>
        </div>
      </div>
      ...
      <div class="relative">
        ...
        <div id="curriculumDropdownMenu" class="hidden absolute left-0 w-full z-10 flex-col bg-white border border-blue-200 rounded shadow-lg mt-1">
          <a href="curi_it.php" class="py-2 px-6 text-blue-700 hover:bg-blue-50 border-b border-blue-200 first:rounded-t">psychology</a>
      ...
      <div class="relative">
        ...
        <div id="fiscalYearDropdownMenu" class="hidden absolute left-0 w-full z-10 flex flex-col bg-white border border-blue-200 rounded shadow-lg mt-1">
          <a href="dashboard2.php" class="py-2 px-6 text-blue-700 hover:bg-blue-50 border-b border-blue-200">All Years</a>
          <?php foreach ($fiscal_years_list as $year): ?>
            <a href="?fiscal_year=<?php echo urlencode($year); ?>" class="py-2 px-6 text-blue-700 hover:bg-blue-50 border-b border-blue-200 last:border-b-0"><?php echo htmlspecialchars($year); ?></a>
          <?php endforeach; ?>
        </div>
      </div>
    </nav>
  ...
  <div class="absolute top-4 right-48 flex items-center space-x-4">
    <div class="flex items-center space-x-4">
      <a href="notification_page.php" class="bg-white bg-opacity-70 p-2 rounded-full shadow hover:bg-opacity-100 transition-all duration-200 relative">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-[#1fbdd2]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
        </svg>
      </a>
  ...
  <div class="bg-white bg-opacity-90 rounded-lg p-6 shadow-md">
    <div class="flex justify-center mb-6">
      <img src="as.png" alt="DCI Logo" class="h-32 w-auto">
    </div>
    <h1 class="text-3xl font-bold text-center text-[#1fbdd2] mb-8">Department of Arts and Sciences</h1>

    <!-- Dashboard Cards -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
      <div class="bg-white text-[#1fbdd2] border-2 border-[#1fbdd2] p-6 rounded-lg shadow-lg">
      ...
      <a href="list.php?program=Psychology" class="bg-white text-[#1fbdd2] border-2 border-[#1fbdd2] p-6 rounded-lg shadow-lg block hover:bg-[#e6fbfe] transition">
        <div class="text-xl font-semibold">Psychology</div>
    ...
    <div class="bg-white bg-opacity-90 rounded-lg p-6 shadow-md mt-10" style="position: relative; z-index: 10;">
      <h2 class="text-2xl font-bold text-[#1fbdd2] mb-6">Statistics Overview</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-[#e6fbfe] p-4 rounded shadow">
          <div class="text-lg font-semibold text-[#0f94a7]">Curriculum in Operation</div>
          <div class="text-2xl font-bold mt-2"><?php echo $curriculum_count; ?></div>
        </div>
        <div class="bg-[#e6fbfe] p-4 rounded shadow">
          <div class="text-lg font-semibold text-[#0f94a7]">Total Population</div>
          <div class="text-2xl font-bold mt-2"><?php echo number_format($total_population); ?></div>
        </div>
         <a href="regularstu.php" class="block bg-[#e6fbfe] p-4 rounded shadow hover:bg-[#d0f5fb] transition duration-200 transform hover:scale-105">
          <div class="text-lg font-semibold text-[#0f94a7]">Regular Students</div>
          <div class="text-2xl font-bold mt-2">
            <?php echo number_format($regular_count); ?>
          </div>
        </a>
        <a href="irregularstu.php" class="block bg-[#e6fbfe] p-4 rounded shadow hover:bg-[#d0f5fb] transition duration-200 transform hover:scale-105">
          <div class="text-lg font-semibold text-[#0f94a7]">Irregular Students</div>
          <div class="text-2xl font-bold mt-2">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div>
          <h3 class="text-lg font-semibold text-[#0f94a7] mb-2">Gender Distribution</h3>
    ...
    <div class="bg-white bg-opacity-90 rounded-lg p-6 shadow-md mt-10 sticky top-0">
      <h2 class="text-2xl font-bold text-[#1fbdd2] mb-6">Year Level Regular/Irregular Pie Charts</h2>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Program Enrollment Chart (Psychology only)
      const programCtx = document.getElementById('programChart').getContext('2d');
      new Chart(programCtx, {
        type: 'pie',
        data: {
          labels: [
            'Psychology (<?php echo $bsit_count; ?>)'
          ],
          datasets: [{
            data: [
              <?php echo $bsit_count; ?>
      ...
      new Chart(statusCtx, {
        ...
        data: {
          ...
          datasets: [{
            ...
            borderColor: ['rgba(31, 189, 210, 1)', 'rgba(255, 159, 64, 1)'],
            borderWidth: 1
          }]
        },
        options: {
