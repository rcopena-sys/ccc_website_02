...
// Fetch Psychology students only (based on students_db.programs)
$students = $conn->query("
    SELECT s.id, CONCAT(s.firstname, ' ', s.lastname) as name, s.student_id 
    FROM signin_db s
    INNER JOIN students_db st ON st.student_id = s.student_id
    WHERE s.role_id IN (4, 5, 6)
      AND st.programs LIKE '%Psychology%'
    ORDER BY s.lastname, s.firstname
")->fetch_all(MYSQLI_ASSOC);

// Fetch available Psychology curriculums only
...
// Fetch current assignments (Psychology only)
$assignments = $conn->query("
    ...
    WHERE ac.program LIKE '%Psychology%'
")->fetch_all(MYSQLI_ASSOC);
?>
