    ...
    if ($handle) {
        ...
        while (($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
            ...
            if ($checkStmt->num_rows > 0) {
                ...
                $checkStmt->close();
                $stmt = $conn->prepare("UPDATE students_db SET student_name = ?, email = ?, curriculum = ?, classification = ?, category = ?, programs = ? WHERE student_id = ?");
                if ($stmt) {
                    $stmt->bind_param("sssssss", $student_name, $email, $curriculum, $classification, $category, $programs, $student_id);
                    if ($stmt->execute()) {
                        $success++;
                    } else {
                        $fail++;
                ...
                $checkStmt->close();
                $stmt = $conn->prepare("INSERT INTO students_db (student_id, student_name, email, curriculum, classification, category, programs) VALUES (?, ?, ?, ?, ?, ?, ?)");
                if ($stmt) {
                    $stmt->bind_param("sssssss", $student_id, $student_name, $email, $curriculum, $classification, $category, $programs);
                    if ($stmt->execute()) {
                        $success++;
                    } else {
                        $fail++;
