...
$isIrregular = checkIrregularStatus($conn, $student_id, 4, 1) || checkIrregularStatus($conn, $student_id, 4, 2);
...
<head>
    ...
    <title>BSCS Prospectus - Fourth Year</title> <link rel="icon" type="image/x-icon" href="favicon.ico">
...
<body>
    ...
    <div class="sidebar">
        <div class="d-flex flex-column align-items-center mb-3">
            <img src="<?php echo !empty($student['profile_image']) ? htmlspecialchars($student['profile_image']) : 'css.png'; ?>" 
                 alt="Profile" 
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item mb-2">
                <a href="cs1st.php" class="nav-link text-white active">
    ...
    <div class="main-content">
        ...
        <div class="prospectus-container">
            ...
            <div class="student-info mb-4">
                <div class="row">
                <div class="program-title">Bachelor of Science in Computer Science</div>
                    <div class="col-md-6">
        ...
        // Calculate total units for BSCS program - Fourth Year
        $totalProgramUnits = 0;
        $completedUnits = 0;
        
        try {
            // First, get all third year courses from irregular_db to calculate total units
            ...
            // Get all fourth year courses from irregular_db (4-1)
            $whereClause = '';
            if (in_array('year_level', $columns) && in_array('semester', $columns)) {
                if (!empty($studentId)) {
                    $whereClause = " WHERE student_id = '$studentId' AND year_level = 4 AND semester = 1";
                } else {
                    $whereClause = " WHERE year_level = 4 AND semester = 1";
                }
            }
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                } elseif ($semesterKey === '4-2') {
                    $second_sem_units += $units;
                }
                
                // Only add to courses array if it's fourth year
...
echo '<!-- Processing: ' . htmlspecialchars($row['course_code']) . ' (Semester: ' . $semester . ') -->';
            ...
            // Now get second semester (4-2) courses separately
            $secondSemWhereClause = '';
            if (in_array('year_level', $columns) && in_array('semester', $columns)) {
                if (!empty($studentId)) {
                    $secondSemWhereClause = " WHERE student_id = '$studentId' AND year_level = 4 AND semester = 2";
                } else {
                    $secondSemWhereClause = " WHERE year_level = 4 AND semester = 2";
                }
            }
...
echo '<!-- 4-1 Courses: ' . count($courses['4-1']) . ' found -->';
foreach ($courses['4-1'] as $course) {
    echo '<!-- 4-1 Course: ' . htmlspecialchars($course['course_code']) . ' -->';
}
echo '<!-- 4-2 Courses: ' . count($courses['4-2']) . ' found -->';
foreach ($courses['4-2'] as $course) {
    echo '<!-- 4-2 Course: ' . htmlspecialchars($course['course_code']) . ' -->';
}
            // Fallback to empty arrays if there's an error
            $courses = [
                '4-1' => [],
            ...
            <div class="table-responsive">
                <table class="curriculum-table">
                    ...
                    <tbody>
                        <?php
                        $rowNum = 1;
                        // Track failed subjects from first semester so we can block dependent second-semester courses
                        $failedFirstSem = [];
                        foreach ($courses['4-1'] as $course) {
            ...
            <div class="table-responsive">
                <table class="curriculum-table">
                    ...
                    <tbody>
                        ...
                        $failedFirstSem = [];
                        foreach ($courses['4-1'] as $firstSemCourse) {
        ...
        <?php
        // Decision support for Year 4: compute average, failed/incomplete counts and show decision
        $numericSum = 0.0;
        $numericCount = 0;
        $failedCount = 0;
        $incompleteCount = 0;
        ...
        echo '<h5 style="margin-top:0;color:#2c3e50;border-bottom:1px solid #dee2e6;padding-bottom:8px;margin-bottom:12px;">Academic Decision Summary - Fourth Year BSCS</h5>';
