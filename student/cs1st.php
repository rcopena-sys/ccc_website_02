...
function tryFlexibleMatching($conn, $tableName, $subjectCodes, &$completedUnits, $columns) {
    ...
    $sql = "SELECT course_code, " . 
          ...
          FROM `$tableName`
          WHERE program = 'BSCS' 
          AND (year_semester = '1-1' OR year_semester = '1-2')
...
<head>
    ...
    <title>BSCS Prospectus - First Year</title> <link rel="icon" type="image/x-icon" href="favicon.ico">
...
<body>
    ...
    <div class="main-content">
        ...
        // Calculate total units for BSCS program - First Year
        $totalProgramUnits = 0;
        $completedUnits = 0;
        ...
        try {
            ...
            // Get all first year courses from irregular_db
            $whereClause = '';
            if (in_array('year_level', $columns) && in_array('semester', $columns)) {
                if (!empty($studentId)) {
                    $whereClause = " WHERE student_id = '$studentId' AND year_level = 1 AND semester = 1";
                } else {
                    $whereClause = " WHERE year_level = 1 AND semester = 1";
                }
            }
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                } elseif ($semesterKey === '1-2') {
                    $second_sem_units += $units;
                }
                
                // Only add to courses array if it's first year
...
echo '<!-- Processing: ' . htmlspecialchars($row['course_code']) . ' (Semester: ' . $semester . ') -->';
            ...
            // Now get second semester (1-2) courses separately
            $secondSemWhereClause = '';
            if (in_array('year_level', $columns) && in_array('semester', $columns)) {
                if (!empty($studentId)) {
                    $secondSemWhereClause = " WHERE student_id = '$studentId' AND year_level = 1 AND semester = 2";
                } else {
                    $secondSemWhereClause = " WHERE year_level = 1 AND semester = 2";
                }
            }
...
echo '<!-- 1-1 Courses: ' . count($courses['1-1']) . ' found -->';
foreach ($courses['1-1'] as $course) {
    echo '<!-- 1-1 Course: ' . htmlspecialchars($course['course_code']) . ' -->';
}
echo '<!-- 1-2 Courses: ' . count($courses['1-2']) . ' found -->';
foreach ($courses['1-2'] as $course) {
    echo '<!-- 1-2 Course: ' . htmlspecialchars($course['course_code']) . ' -->';
}
            // Fallback to empty arrays if there's an error
            $courses = [
                '1-1' => [],
            ...
            <div class="table-responsive">
                <table class="curriculum-table">
                    ...
                    <tbody>
                        ...
                        $failedFirstSem = [];
                        foreach ($courses['1-1'] as $firstSemCourse) {
        ...
        $yearSubjects = array_merge($courses['1-1'] ?? [], $courses['1-2'] ?? []);
        ...
        echo '<h5 style="margin-top:0;color:#2c3e50;border-bottom:1px solid #dee2e6;padding-bottom:8px;margin-bottom:12px;">Academic Decision Summary - First Year BSCS</h5>';
        ...
        <div class="modal-dialog modal-fullscreen">
            <div class="modal-content">
                <div class="modal-header" style="background-color: #0d6efd; color: white;">
                    <h5 class="modal-title" id="fifthYearModalLabel">Fifth Year - BSCS Prospectus</h5>
