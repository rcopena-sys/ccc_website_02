...
function tryFlexibleMatching($conn, $tableName, $subjectCodes, &$completedUnits, $columns) {
    ...
    $sql = "SELECT course_code, " . 
          ...
          FROM `$tableName`
          WHERE program = 'BSCS' 
          AND (year_semester = '2-1' OR year_semester = '2-2')
...
<head>
    ...
    <title>BSCS Prospectus - Second Year</title> <link rel="icon" type="image/x-icon" href="favicon.ico">
...
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
        ...
        // Calculate total units for BSCS program - Second Year
        $totalProgramUnits = 0;
        $completedUnits = 0;
        
        try {
            // First, get all second year courses from irregular_db to calculate total units
            ...
            // Get all second year courses from irregular_db (2-1)
            $whereClause = '';
...
echo '<!-- 2-1 Courses: ' . count($courses['2-1']) . ' found -->';
foreach ($courses['2-1'] as $course) {
    echo '<!-- 2-1 Course: ' . htmlspecialchars($course['course_code']) . ' -->';
}
echo '<!-- 2-2 Courses: ' . count($courses['2-2']) . ' found -->';
foreach ($courses['2-2'] as $course) {
    echo '<!-- 2-2 Course: ' . htmlspecialchars($course['course_code']) . ' -->';
}
            // Fallback to empty arrays if there's an error
            $courses = [
                '2-1' => [],
                ...
                <table class="curriculum-table">
                    ...
                    <tbody>
                        ...
                        foreach ($courses['2-2'] as $course) {
                            ...
                            // Units are now calculated in the filtering step
                            $displayGrade = '-';
                            $hasGrade = !empty($grade);
        ...
        <?php
        // Decision support for Year 2: compute average, failed/incomplete counts and show decision
        $numericSum = 0.0;
        $numericCount = 0;
        $failedCount = 0;
        $incompleteCount = 0;
        ...
        echo '<h5 style="margin-top:0;color:#2c3e50;border-bottom:1px solid #dee2e6;padding-bottom:8px;margin-bottom:12px;">Academic Decision Summary - Second Year BSCS</h5>';
