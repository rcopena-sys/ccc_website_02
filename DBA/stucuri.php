...
// Fetch students (only BSA / BSAIS, with program from students_db)
$students = $conn->query("
        SELECT s.id,
                     ...
                     sd.programs
        FROM signin_db s
        LEFT JOIN students_db sd ON s.student_id = sd.student_id
        WHERE s.role_id IN (4, 5, 6)
            AND sd.programs IN ('BSA','BSAIS')
...
// Fetch current assignments (only BSA / BSAIS, show student program)
$assignments = $conn->query("
    SELECT ac.*,
           CONCAT(s.firstname, ' ', s.lastname) AS student_name,
           s.student_id,
           sd.programs AS student_program,
    ...
    WHERE sd.programs IN ('BSA','BSAIS')
")->fetch_all(MYSQLI_ASSOC);
?>
                            ...
                            <?php if (count($assignments) > 0): ?>
                                <?php foreach ($assignments as $assign): ?>
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <?= htmlspecialchars($assign['student_program'] ?? $assign['program']) ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
