...
// Load available fiscal years for BSA from curriculum table
$availableFiscalYears = [];
$fySql = "SELECT DISTINCT fiscal_year FROM curriculum WHERE program = 'BSA' AND fiscal_year IS NOT NULL AND fiscal_year <> '' ORDER BY fiscal_year DESC";
...
// Fetch curriculum data for BSA
$curriculumData = [];
if ($selectedFiscalYear) {
    $sql = "SELECT * FROM curriculum WHERE program = 'BSA' AND fiscal_year = ? ORDER BY year_semester, course_code";
...
<head>
    ...
    <style>
        /* Force blue palette on this page (override dba-theme.css) */
        body {
            background-color: #eff6ff !important; /* light blue */
        }

        aside {
            background: linear-gradient(180deg, #2563eb, #1d4ed8) !important; /* blue sidebar */
        }
        ...
        /* Force Student dropdown button to blue (override yellow theme) */
        #studentDropdownBtn {
            background-color: #2563eb !important;
            color: #ffffff !important;
        }

        #studentDropdownBtn:hover,
        #studentDropdownBtn:focus {
            background-color: #1d4ed8 !important;
            color: #ffffff !important;
        }
...
<body class="flex h-screen">
    ...
    <nav class="space-y-4 mt-4">
        ...
        <div class="relative">
            ...
            <div id="curriculumDropdownMenu" class="hidden absolute left-0 w-full z-10 flex flex-col bg-white border border-blue-200 rounded shadow-lg mt-1">
                <a href="curi_cs.php" class="py-2 px-6 text-blue-700 hover:bg-blue-100 border-b border-blue-100 first:rounded-t">BSA Curriculum</a>
                <a href="curi_it.php" class="py-2 px-6 text-blue-700 hover:bg-blue-100 last:rounded-b bg-blue-100">BSAIS Curriculum</a>
            </div>
        </div>
    </nav>
    <script>
  ...
  <main class="flex-1 overflow-y-scroll p-8 space-y-12 main-content">
    ...
    <?php endif; ?>
                ...
                <a href="add_curriculum.php" class="inline-block action-btn-primary px-4 py-2 rounded-md no-print">
      + Add Curriculum
    </a>
                <a href="remove_duplicates.php" class="inline-block action-btn-secondary px-4 py-2 rounded-md ml-2 no-print">
      Remove Duplicates
    </a>
    ...
    <section class="space-y-10">
      ...
      <div class="mt-6">
        <h2 class="text-2xl font-bold">BACHELOR OF SCIENCE IN ACCOUNTING INFORMATION</h2>
      </div>
    </header>
