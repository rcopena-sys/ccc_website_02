...
// For DBA, always work with BSA and BSAIS students only
$base_where = $where_clause
  ? $where_clause . " AND programs IN ('BSA','BSAIS')"
  : " WHERE programs IN ('BSA','BSAIS')";
...
try {
    ...
    // 2. Get program-wise counts (BSA and BSAIS only)
    $bscs_count = 0; // will hold BSAIS
    $bsit_count = 0; // will hold BSA
    
    // Count from students_db
    $program_result = $conn->query("SELECT programs, COUNT(*) as total FROM students_db " . ($where_clause ? $where_clause . " AND programs IN ('BSA','BSAIS')" : " WHERE programs IN ('BSA','BSAIS')") . " GROUP BY programs");
    if ($program_result) {
      while ($row = $program_result->fetch_assoc()) {
        if ($row['programs'] === 'BSAIS') {
          $bscs_count += (int)$row['total'];
        } elseif ($row['programs'] === 'BSA') {
    ...
    // 3. Get curriculum count for BSA and BSAIS by fiscal year
    $curriculum_stats = [];
    $curriculum_result = $conn->query("SELECT fiscal_year, COUNT(DISTINCT program) as count 
      FROM curriculum 
      WHERE program IN ('BSA','BSAIS') 
    ...
    if ($curriculum_result && $curriculum_result->num_rows > 0) {
        ...
        // Fallback to total distinct BSA and BSAIS programs
    ...
    // 4. Get enrollment by year and semester (BSA/BSAIS only)
    ...
    // Get total population from students_db (BSA/BSAIS only)
    ...
    // Get total population from signin_db (BSA and BSAIS only, if roles exist)
    $signin_total_result = $conn->query("SELECT COUNT(*) as total FROM signin_db s LEFT JOIN roles r ON s.role_id = r.role_id WHERE r.role_name IN ('BSA','BSAIS')");
    ...
    // 6. Get gender breakdown (BSA/BSAIS only)
    ...
    // 7. Get academic year breakdown (BSA/BSAIS only)
    ...
    // 8. Get total population (count distinct BSA/BSAIS student IDs to avoid duplicates)
    $total_result = $conn->query("SELECT COUNT(DISTINCT student_id) as total FROM students_db" . $base_where);
    if ($total_result) {
        $total_population = (int)$total_result->fetch_assoc()['total'];
    }

    // 9. Get fiscal year count (for BSA/BSAIS only)
    $fiscal_year_count = 0;
    $fiscal_year_result = $conn->query("SELECT COUNT(DISTINCT fiscal_year) as total FROM students_db WHERE programs IN ('BSA','BSAIS')");
    if ($fiscal_year_result) {
        $fiscal_year_count = (int)$fiscal_year_result->fetch_assoc()['total'];
    }

    // 10. Get program enrollment for chart (BSA/BSAIS only)
...
// By curriculum (BSA/BSAIS only)
$curriculum = [];
$res = $conn->query("SELECT curriculum, COUNT(*) as total FROM students_db" . $base_where . " GROUP BY curriculum");
while ($row = $res->fetch_assoc()) {
    $curriculum[$row['curriculum']] = $row['total'];
}

// By classification (BSA/BSAIS only)
$classifications = [];
$res = $conn->query("SELECT classification, COUNT(*) as total FROM students_db" . $base_where . " GROUP BY classification");
while ($row = $res->fetch_assoc()) {
...
// By programs (BSA/BSAIS only)
$programs = [];
$res = $conn->query("SELECT programs, COUNT(*) as total FROM students_db" . $base_where . " GROUP BY programs");
while ($row = $res->fetch_assoc()) {
...
foreach ($year_levels as $level) {
  ...
  $res = $conn->query("SELECT classification, COUNT(*) as total FROM students_db " . ($where_clause ? $where_clause . " AND programs IN ('BSA','BSAIS')" : " WHERE programs IN ('BSA','BSAIS')") . " GROUP BY classification");
    if ($res) {
        while ($row = $res->fetch_assoc()) {
...
<body class="bg-blue-100 min-h-screen">
 ...
 <aside id="sidebar" class="w-64 bg-blue-600 text-white p-6 flex flex-col transform transition-transform duration-300 ease-in-out">
    <div class="flex flex-col items-center">
      <div class="w-40 h-40 rounded-full overflow-hidden border-4 border-white shadow-lg mb-4 flex items-center justify-center">
        <img src="bsa.png" alt="DBA Logo" class="max-w-full max-h-full object-contain" />
      </div>
      <h2 class="font-semibold text-lg">DBA</h2>
    ...
    <nav class="space-y-4 mt-4">
      ...
      <div class="relative">
        ...
        <div id="curriculumDropdownMenu" class="hidden absolute left-0 w-full z-10 flex-col bg-white border border-blue-200 rounded shadow-lg mt-1">
          <a href="curi_it.php" class="py-2 px-6 text-blue-700 hover:bg-blue-100 border-b border-blue-100 first:rounded-t">BSA</a>
          <a href="curi_cs.php" class="py-2 px-6 text-blue-700 hover:bg-blue-100 last:rounded-b">BSAIS</a>
        </div>
      </div>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Program Enrollment Chart (BSA and BSAIS)
      const programCtx = document.getElementById('programChart').getContext('2d');
      new Chart(programCtx, {
        type: 'pie',
        data: {
          labels: [
            'BSAIS (<?php echo $bscs_count; ?>)', 
            'BSA (<?php echo $bsit_count; ?>)'
          ],
          datasets: [{
            data: [
              <?php echo $bscs_count; ?>, 
