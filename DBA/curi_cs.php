...
// Load available fiscal years for BSAIS from curriculum table
$availableFiscalYears = [];
$fySql = "SELECT DISTINCT fiscal_year FROM curriculum WHERE program = 'BSAIS' AND fiscal_year IS NOT NULL AND fiscal_year <> '' ORDER BY fiscal_year DESC";
...
// Fetch curriculum data for BSAIS
$curriculumData = [];
if ($selectedFiscalYear) {
    $sql = "SELECT * FROM curriculum WHERE program = 'BSAIS' AND fiscal_year = ? ORDER BY year_semester, course_code";
...
<head>
    <meta charset="UTF-8">
    <title>BSAIS Curriculum</title> <link rel="icon" type="image/x-icon" href="favicon.ico">
...
<aside class="w-64 bg-blue-600 text-white p-6 flex flex-col no-print">
    <div class="flex flex-col items-center">
        <img src="dba.png" class="rounded-full " alt="DCI Logo" />
        <h2 class="mt-4 font-semibold text-lg">DBA</h2>
    ...
    <nav class="space-y-4 mt-4">
        ...
        <div class="relative">
            ...
            <div id="curriculumDropdownMenu" class="hidden absolute left-0 w-full z-10 flex flex-col bg-white border border-blue-200 rounded shadow-lg mt-1">
                <a href="curi_cs.php" class="py-2 px-6 text-blue-700 hover:bg-blue-100 border-b border-blue-100 first:rounded-t bg-blue-100">BSAIS Curriculum</a>
                <a href="curi_it.php" class="py-2 px-6 text-blue-700 hover:bg-blue-100 last:rounded-b">BSA Curriculum</a>
            </div>
        </div>
    </nav>
    <script>
...
<main class="flex-1 overflow-y-scroll p-8 space-y-12 main-content">
    ...
    <h2 class="text-3xl font-bold mb-4 no-print">BSAIS Curriculum (Prospectus)</h2>
    ...
    <?php if (!isset($_GET['fiscal_year']) || !$_GET['fiscal_year']): ?>
        <div class="mb-8 p-4 bg-blue-500 text-white rounded no-print">Please select a fiscal year to view the curriculum.</div>
    <?php else: ?>
        <div class="mb-8 p-4 bg-blue-500 text-white rounded no-print">Showing curriculum for fiscal year <strong><?= htmlspecialchars($_GET['fiscal_year']) ?></strong>.</div>
    <?php endif; ?>
    ...
    <section class="space-y-10">
        ...
        <div class="mt-6">
            <h2 class="text-2xl font-bold">BACHELOR OF SCIENCE IN ACCOUNTING INFORMATION SYSTEM</h2>
        </div>
    </header>
