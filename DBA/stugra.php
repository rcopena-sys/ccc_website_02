...
if (!empty($_GET['search_student']) || !empty($_GET['search_course'])) {
    ...
    if (!empty($search_student)) {
        $where_conditions[] = "g.student_id LIKE ?";
        $params[] = "%$search_student%";
        $types .= 's';
    }
    
    if (!empty($search_course)) {
        $where_conditions[] = "g.course_code LIKE ?";
        $params[] = "%$search_course%";
        $types .= 's';
    }
    
    // Always limit to BSA and BSAIS programs via students_db
    $where_conditions[] = "s.programs IN ('BSA','BSAIS')";
    
    if (!empty($where_conditions)) {
        $sql = "SELECT g.*, s.programs FROM grades_db g LEFT JOIN students_db s ON g.student_id = s.student_id WHERE " . implode(' AND ', $where_conditions) . " ORDER BY g.student_id, g.course_code";
...
// Get student IDs for dropdown (only BSA / BSAIS students)
$student_ids = [];
$student_result = $conn->query("SELECT DISTINCT g.student_id FROM grades_db g LEFT JOIN students_db s ON g.student_id = s.student_id WHERE s.programs IN ('BSA','BSAIS') ORDER BY g.student_id");
if ($student_result) {
...
<head>
    ...
    <style>
        /* Page background to match blue DBA theme */
        body {
        ...
        /* Table header in blue instead of dark gray */
...
<body>
    ...
    <div class="container py-5 grades-wrapper mt-5">
        <div class="row mb-4">
            <div class="col-12">
        ...
        <div class="mt-5">
            ...
            <div class="table-responsive">
                <table class="table table-sm table-bordered table-hover">
                    ...
                    <tbody>
                    <?php
                    $display_data = $search_performed
                        ? $search_results
                        : $conn->query("SELECT g.*, s.programs FROM grades_db g LEFT JOIN students_db s ON g.student_id = s.student_id WHERE s.programs IN ('BSA','BSAIS') ORDER BY g.student_id, g.course_code")->fetch_all(MYSQLI_ASSOC);
                    ...
                    ?>
                            <tr class="<?= $grade_class ?>">
                                ...
                                <td><?= htmlspecialchars($row['programs'] ?? 'N/A') ?></td>
