...
function isRegularStudent($studentData) {
  ...
  function normalizeDbaProgram(string $program): string {
    ...
    if (strpos($program, 'BSAIS') !== false || strpos($program, 'BSCS') !== false) {
      return 'BSAIS';
    }

    if (strpos($program, 'BSA') !== false || strpos($program, 'BSIT') !== false) {
      return 'BSA';
    }
    ...
    if (isset($data['action']) && $data['action'] === 'bulk_save_irregular') {
        ...
        try {
          ...
          if (!empty($data['program'])) {
            $effectiveProgram = normalizeDbaProgram((string)$data['program']);
          }
          if ($effectiveProgram === '') {
            $effectiveProgram = 'BSAIS'; // Fallback when not specified
          }
...
$program = '';
if (strpos($studentClassification, 'irregular') !== false && in_array(normalizeDbaProgram($formProgram), ['BSA', 'BSAIS'], true)) {
  $program = normalizeDbaProgram($formProgram);
}
...
$program = normalizeDbaProgram($program);
if ($program !== 'BSA' && $program !== 'BSAIS') {
...
foreach ($ysOrder as $ys) {
  ...
  $unitLimits['BSA'][$ys] = ['max' => $limit];
  $unitLimits['BSAIS'][$ys] = ['max' => $limit];
}
...
<head>
  ...
  <title>DBA Evaluation</title> <link rel="icon" type="image/x-icon" href="favicon.ico">
...
<body class="<?php echo $isModalView ? 'modal-view' : ''; ?>">
    ...
    <ul class="nav flex-column">
      ...
      <li class="nav-item mb-2">
        <a href="stueval.php" class="nav-link active">
          <i class="bi bi-journal-text"></i> DBA Evaluation
        </a>
      </li>
      <li class="nav-item mb-2">
  ...
  <div class="main-content">
    ...
    <h1 class="h3 mb-0">DBA Evaluation</h1>
  </div>
      ...
      <div class="row mt-3">
        <div class="col-md-12 d-flex justify-content-start">
          <button type="button" id="openOtherProgramModal" class="btn btn-outline-primary">
            ...
            Add Subject from Other DBA Program
          </button>
        </div>
      </div>
      <?php endif; ?>
...
<!-- Other DBA Program Subjects Modal (for irregular students) -->
<div class="modal fade" id="otherProgramModal" tabindex="-1" aria-labelledby="otherProgramModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="otherProgramModalLabel">Add Subject from Other DBA Program</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="row mb-3">
          <div class="col-md-4">
            ...
            <div class="form-control bg-light">
              ...
              <span class="text-muted small d-block">(Student's enrolled DBA program)</span>
            </div>
          </div>
          <div class="col-md-3">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const selectedProgram = '<?= htmlspecialchars($program ?: 'BSAIS') ?>';
    const unitLimits = <?= json_encode($unitLimits) ?>;
    ...
    const limitProgramKey = '<?= htmlspecialchars($program ?: 'BSAIS') ?>';
